...
This template will let you do the following:
 ...
 1. Add your Dockerfile to a Github repository
...
Github repository that has a Dockerfile.

## Reproducible Content Generated

 - **container** you generate a Docker Hub container from your Dockerfile, ready for use wherever you can run the container. The container always provides a latest, but also a tagged version coinciding with the first 10 characters of each merged commit.
 ...
 - **inspection** using the [container-diff](https://github.com/GoogleContainerTools/container-diff) tool, every tag of a container has apt, pip, and files recorded. Yes, this means that you can programatically find containers based on software. @vsoch will be developing a command line tool (TBA) to help with this.
...
Today you will be doing the following:
  ...
  1.  Fork and clone the share-docker Github repository to obtain
...
First, fork the [docker-share](https://www.github.com/vsoch/docker-share/)
Github repository to your account, and clone the branch.

```bash
git clone https://www.github.com/<username>/docker-share
git clone git@github.com:<username>/docker-share.git
```
...
and build your container. There is also a [template.html](template.html)
...
build and deploy, including:
 ...
 1.  clone of the repository with the Dockerfile that you specify
 2.  build
...
And then proceed with your container's usage, usually a "run" command. For a pre-built working example, try the following:
...
# Then open to http://127.0.0.1:888 to see the notebook, and enter the token that appears in the terminal.
```

When you are done and need to stop and remove the
container:
...
If you want to get help please [post an issue!](https://www.github.com/vsoch/docker-share/issues)
